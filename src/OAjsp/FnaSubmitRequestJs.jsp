...
<%
if(true){
	request.getRequestDispatcher("/fna/template/FnaSubmitRequestJs_e9.jsp").forward(request,response);
	return;
}
...
if(user==null){
	response.sendRedirect(weaver.general.GCONST.getContextPath()+"/notice/noright.jsp") ;
	return ;
}
...
String main_fieldIdSfysn = Util.null2String(dataMap.get("main_fieldIdSfysn_fieldId"));
...
FnaCommon.getHrmResourceInfo(dataMapHrm, Util.getIntValue(beagenter));
...
%>
<script language="javascript" src="<%=weaver.general.GCONST.getContextPath()%>/fna/js/e8Common_wev8.js?r=20"></script>
<script language="javascript">
var _fnaUserId = "<%=beagenter%>";
...
var main_fieldIdSfysn = "<%=main_fieldIdSfysn %>";
...
function addDetail1Row_callBack(rowJsonObj, _idx){
	...
	try{
		if(fnaRound2(rowJsonObj.subject,0)<=0){
			rowJsonObj.subject = "";
			rowJsonObj.subjectname = "";
		}
	}catch(ex0){}
	try{
	...
	if(dt1_subject==fieldid || dt1_organizationtype==fieldid || dt1_organizationid==fieldid || dt1_budgetperiod==fieldid){
		...
		if(_isMainField && dt1_haveIsDtlField){
			...
			for(var i=0;i<_xm_array3.length;i++){
				...
				if(!isNaN(_idx) && _idx>=0){
					if(dt1_organizationid==fieldid || dt1_subject==fieldid){
...
function getOrgSpanAndInitOrgIdBtn_defalutFieldValue(insertindex){
	if(main_fieldIdFysqlc!=null && main_fieldIdFysqlc!=""){
		var _need_do_getOrgSpanAndInitOrgIdBtn = false;
		if(dt1_organizationid_isDtl=="1"){
			if(fnaRound2(insertindex,0)>=0){
				_need_do_getOrgSpanAndInitOrgIdBtn = true;
			}
		}else{
			_need_do_getOrgSpanAndInitOrgIdBtn = true;
		}
		if(_need_do_getOrgSpanAndInitOrgIdBtn){
			var orgtype = getWfMainAndDetailFieldValueForPc(dt1_organizationtype, dt1_organizationtype_isDtl, insertindex);
...
function getOrgSpanAndInitOrgIdBtn(orgtype,orgid,insertindex,addDetail1Row){
	...
	jQuery.ajax({
		...
		success: function do4Success(msg){ 
			...
			if(dt1_organizationid_isDtl=="1"){
				if(fnaRound2(insertindex,0)>=0){
					orgid_obj = jQuery("#field1"+dt1_organizationid+"_"+insertindex);
				}
			}else{
				orgid_obj = jQuery("#field1"+dt1_organizationid);
			}
			if(orgid_obj!=null && orgid_obj.length){
				var _orgid_isReadOnly = true;
				var viewtype = orgid_obj.attr("viewtype");
				if(viewtype){
					_orgid_isReadOnly = false;
				}else{
					var _isedit = orgid_obj.attr("_isedit");
					if(_isedit){
						_orgid_isReadOnly = false;
					}
				}
				if(_orgid_isReadOnly){
					setWfMainAndDetailFieldValueForPc(orgid, dt1_organizationid, dt1_organizationid_isDtl, insertindex);
					setWfMainAndDetailFieldSpanValueForPc(orgSpan, dt1_organizationid, dt1_organizationid_isDtl, insertindex);
					getFnaInfoData(insertindex);
				}else{
...
//获取是否在流程html模板中有放，字段对应中对应了显示预算信息的字段。
var ___needGetFnaInfo = null;
function needGetFnaInfo(){
	if(___needGetFnaInfo==null){
		
		if(dt1_haveIsDtlField){
			var _xm_array = jQuery("input[name='check_node_0']");
			var _xm_array_length = _xm_array.length;
			if(_xm_array_length <= 0){
				return false;
			}
		}
		
		var dt1_fieldId_isDtl_array = [dt1_hrmremain, dt1_deptremain, dt1_subcomremain, dt1_fccremain];
		var dt1_fieldId_isDtl_array_len = dt1_fieldId_isDtl_array.length;
		for(var i=0;i<dt1_fieldId_isDtl_array_len;i++){
			var _fieldId = dt1_fieldId_isDtl_array[i];
			if(dt1_haveIsDtlField){
				var _obj = jQuery("span[id^='field"+_fieldId+"_']");
				if(_obj.length>0){
					___needGetFnaInfo = true;
					return true;
				}
			}else{
				var _obj = jQuery("span[id='field"+_fieldId+"span']");
				if(_obj.length>0){
					___needGetFnaInfo = true;
					return true;
				}
		...
		___needGetFnaInfo = false;
		return false;
	}else{
		return ___needGetFnaInfo;
	}
}
//获取所有明细行预算信息
function getFnaInfoDataBatch(){
	...
	var _xm_array_length = _xm_array.length;
	if(_xm_array_length > 0){
		if(needGetFnaInfo() && _fnaSubmitRequestJs_global_getFnaInfoData_callBack_flag){
			_fnaSubmitRequestJs_global_getFnaInfoDataBatch_ajaxTimes = 0;
			_fnaSubmitRequestJs_global_getFnaInfoDataBatch_success_jsonDataArray = [];
...
//jQuery(document).ready(function(){
	...
	function getPoststr(){
		...
		if(dt1_haveIsDtlField){
			...
			for(var i0=0;i0<_xm_array.length;i0++){
				try{
					...
					if(!isNaN(_idx) && _idx>=0){
						...
						var _value_main_fieldIdSfysn = getWfMainAndDetailFieldValueForPc(main_fieldIdSfysn, "0", "");//主表字段 下拉框的页面真值,不是数据库的
						if(_value_main_fieldIdSfysn==""){_value_main_fieldIdSfysn="-100";}//0,1 都是有含义的值,不能设为默认值
					    ...
					    //if(budgetfeetype!="" && orgtype!="" && orgid!="" && applydate!="" && applyamount!=""){
						    ...
						    //alert("_reqId="+_reqId);
						    //alert("_reqDtId="+_reqDtId);
					    	poststr += budgetfeetype+","+ orgtype+","+ orgid+","+ applydate+","+applyamount+","+dtl_id+","+_reqId+","+_reqDtId+","+_value_main_fieldIdSfysn+",postStrEnd";
					    //}
				    }
				}catch(e){}
			}
		    ...
		    //if(budgetfeetype!="" && orgtype!="" && orgid!="" && applydate!="" && applyamount!=""){
		    	...
		    	poststr += budgetfeetype+","+ orgtype+","+ orgid+","+ applydate+","+applyamount+","+dtl_id+",0,0,"+_value_main_fieldIdSfysn+",postStrEnd";
		    //}
		}
		return poststr;
	}
	...
	fnaifoverJson_doSubmit = function(obj){
		try{
			...
			var _data = "poststr="+poststr+"&poststr2="+poststr2+"&poststr3="+poststr3+"&poststr4="+poststr4+"&requestid="+temprequestid+"&nodeid="+__nodeid+"&workflowid="+__workflowid+"&fysqlc="+fysqlc+"&yfkZfHj="+yfkZfHj;
